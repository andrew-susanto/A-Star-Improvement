...
class AStarGUI(Frame):
    ...
    def draw_initial_map(self):
        ...
        # Draw "guide line"
        x_start = self.start[1]*self.sqsize + (self.sqsize/2)
        y_start = self.start[0]*self.sqsize + (self.sqsize/2)
        x_end = self.end[1]*self.sqsize + (self.sqsize/2)
        y_end = self.end[0]*self.sqsize + (self.sqsize/2)
        self.board.create_line(x_start, y_start, x_end, y_end, width=3, fill="red")
    ...
    def a_star(self):
        ...
        # Create guide line equation as Ax+By+c=0
        guideline_a = self.end[0] - self.start[0]
        guideline_b = self.start[1] - self.end[1]
        guideline_c = (self.end[1] - self.start[1])*self.start[0] - (self.end[0]-self.start[0])*self.start[1]
        distance_divider = math.sqrt(guideline_a**2 + guideline_b**2) # Used later when calculate distance from "guide line"
        ...
        while len(open_list) > 0:
            ...
            for child in children:
                ...
                # Create the f, g, h, and c values
                start_time = time.time()
                child.c = abs(child.x*guideline_a + child.y*guideline_b + guideline_c) / distance_divider
                child.g = current_node.g + 1
                child.h = abs(child.y - end_node.y)  + abs(child.x - end_node.x) 
                child.f = child.g + child.h + child.c
